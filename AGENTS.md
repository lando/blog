...
- Keep `.vuepress/public/llms.txt` and `.vuepress/public/llms-full.txt` as static public files for the pinned VuePress build. They are copied directly to the production site root during build and are served under the blog base path.
- Update both files periodically when blog content, public URLs, donation/support routes, metadata links, or major Lando Alliance context changes.
